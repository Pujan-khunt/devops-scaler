// Hello World web server in pure Node.js — no dependencies, so the image stays small
// and `npm install` never has to reach the network at build time.
const http = require('http');
const os = require('os');

const PORT = process.env.PORT || 3000;

const page = () => `<!doctype html>
<html lang="en">
<head>
  <meta charset="utf-8">
  <meta name="viewport" content="width=device-width,initial-scale=1">
  <title>Hello World — Node.js</title>
  <style>${STYLE}</style>
</head>
<body>
  <div class="card">
    <div class="badge">● Node.js</div>
    <h1>Hello <span>World</span></h1>
    <p class="sub">Served by a pure Node.js HTTP server running inside a Docker container.</p>
    <dl>
      <dt>Runtime</dt><dd>Node ${process.version}</dd>
      <dt>Platform</dt><dd>${process.platform} / ${process.arch}</dd>
      <dt>Container</dt><dd>${os.hostname()}</dd>
      <dt>Port</dt><dd>${PORT}</dd>
      <dt>Uptime</dt><dd>${process.uptime().toFixed(1)}s</dd>
    </dl>
    <p class="foot">Pujan Khunt &middot; 24BCS10138 &middot; DevOps Homework</p>
  </div>
</body>
</html>`;

const STYLE = `
:root{--bg:#0f1117;--card:#171a23;--line:#252a38;--fg:#e6e9f0;--muted:#8b93a7;--accent:#6cc24a}
*{margin:0;padding:0;box-sizing:border-box}
body{background:var(--bg);color:var(--fg);min-height:100vh;display:grid;place-items:center;
  font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,sans-serif;padding:24px}
.card{background:var(--card);border:1px solid var(--line);border-radius:16px;padding:44px 52px;
  max-width:560px;width:100%;box-shadow:0 24px 60px rgba(0,0,0,.5)}
.badge{display:inline-flex;align-items:center;gap:8px;font-size:12px;font-weight:600;
  letter-spacing:.08em;text-transform:uppercase;color:var(--accent);
  background:rgba(108,194,74,.14);border:1px solid rgba(108,194,74,.3);
  padding:6px 12px;border-radius:999px;margin-bottom:22px}
h1{font-size:44px;line-height:1.1;letter-spacing:-.02em;margin-bottom:10px}
h1 span{color:var(--accent)}
.sub{color:var(--muted);font-size:15px;margin-bottom:28px}
dl{display:grid;grid-template-columns:auto 1fr;gap:10px 18px;font-size:14px;
  border-top:1px solid var(--line);padding-top:22px}
dt{color:var(--muted)}
dd{font-family:ui-monospace,'SF Mono',Menlo,monospace}
.foot{margin-top:24px;padding-top:18px;border-top:1px solid var(--line);color:var(--muted);font-size:13px}
`;

const server = http.createServer((req, res) => {
  if (req.url === '/health') {
    res.writeHead(200, { 'Content-Type': 'application/json' });
    return res.end(JSON.stringify({ status: 'ok', app: 'nodejs-app' }));
  }
  res.writeHead(200, { 'Content-Type': 'text/html; charset=utf-8' });
  res.end(page());
});

server.listen(PORT, '0.0.0.0', () => {
  console.log(`nodejs-app listening on http://0.0.0.0:${PORT}`);
});
